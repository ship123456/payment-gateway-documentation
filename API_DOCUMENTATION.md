...
The APIs enables merchants to create payments, check transaction status, process refunds, and receive real-time payment updates through webhooks.
...
- Use production API keys only for live transactions.
...
Every API response includes a unique request identifier (X-Request-ID) that can be used for debugging and support.
